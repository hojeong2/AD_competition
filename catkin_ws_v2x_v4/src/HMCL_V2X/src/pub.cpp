...
int main(int argc, char **argv)
{
    ...
    while(1){    
        ...
        if (uperSize > 0)
        {
            ...
            if (spat_ptr->id.id == 1 && a < 12)
            {        
                    printf("\033[0;34m [INFO]Receiving KIAPI-1\033[0m \n");
                    
                    HMCL_V2X::HMCL_SPAT msg;
                                        
                    // Main Field
                    msg.data.name="KIAPI-1";
                    msg.data.region=0;
                    msg.data.id = spat_ptr->id.id;
                    msg.data.revision= spat_ptr->revision;
                    msg.data.status=spat_ptr->status.buf[0];
                    msg.data.moy=spat_ptr->moy[0];
                    msg.data.timeStamp=spat_ptr->timeStamp[0];

                int Signum = spat_ptr->states.list.count;
                ...
                {   
                    ...
                    ss<< spat_ptr->states.list.array[k]->movementName->buf[0]<<spat_ptr->states.list.array[k]->movementName->buf[1]<<spat_ptr->states.list.array[k]->movementName->buf[2];

                    msg_state.states= k;
                    msg_state.movementName=ss.str();
                    msg_state.signalGroup= spat_ptr->states.list.array[k]->signalGroup;
                    msg_state.eventState= spat_ptr->states.list.array[k]->state_time_speed.list.array[0]->eventState;
                    msg_state.timing_min_End_Time= spat_ptr->states.list.array[k]->state_time_speed.list.array[0]->timing->minEndTime/10;
                    msg_state.connectionID=spat_ptr->states.list.array[k]->maneuverAssistList->list.array[0]->connectionID;
                    msg_state.pedBicycleDetect=spat_ptr->states.list.array[k]->maneuverAssistList->list.array[0]->pedBicycleDetect[0];
            ...
            a=0; 
            if (spat_ptr->id.id == 2 && a < 12)
            {
                printf("\033[0;34m [INFO]Receiving KIAPI-2\033[0m \n");

                HMCL_V2X::HMCL_SPAT msg;

                // Main Field
                msg.data.name="KIAPI-2";
                msg.data.region=0;
                msg.data.id = spat_ptr->id.id;
                msg.data.revision= spat_ptr->revision;
                msg.data.status=spat_ptr->status.buf[0];
            ...
            a=0; 
            if (spat_ptr->id.id == 3 && a < 12)
            {
                printf("\033[0;34m [INFO]Receiving KIAPI-3\033[0m \n");

                HMCL_V2X::HMCL_SPAT msg;

                // Main Field
                msg.data.name="KIAPI-3";
                msg.data.region=0;
                msg.data.id = spat_ptr->id.id;
                msg.data.revision= spat_ptr->revision;
                msg.data.status=spat_ptr->status.buf[0];
